...
int runTest(void) {
    ...
    // during the test
    for (int i = 0; i < (int)testCollection.size; i++) {
        ...
        if (status != OK) {
            printf("\033[1;31m FAILED:\033[0m Unable to kick off the test");
            break;
        }
        ...
        for (int j = 0; j < (int)assertCollection.size; j++) {
        ...
        if (allTestsPassed) {
            ...
            for (int j = 0; j < (int)assertCollection.size; j++) {
                if (assertCollection.items[j].passed)
                    printf("\tAssert %s has \033[1;32mPASSED\033[0m\n",
                           assertCollection.items[j].assertName);
                else
                    printf("\tAssert %s has \033[1;31mFAILED\033[0m\n",
                           assertCollection.items[j].assertName);
            }
        }
